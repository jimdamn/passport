...
import { MapPin, ChevronRight, Navigation, X } from 'lucide-react';
import { Spinner } from '../../components/ui/Spinner';

// The Places lens - the member business directory, moved intact from the old
// Explore.tsx (category chips, radius/location controls, member cards) per
// AROUND-TOWN-BUILD-PLAN.md §3.1 item 7: "move, don't rewrite." Only the
// header/back-link/entry-cards that used to live around this were dropped -
// those are now the board's greeting and quiet module links row one level up
// in Explore.tsx. The sort line below is the one new addition this lens gets.
...
export default function PlacesLens() {
  const { user } = useAuth();
  ...
  return (
    <div>
      ...
      {/* The trust line - never a ranking claim, never paid placement. */}
      <p style={{ margin: '0 0 12px', fontSize: '0.78rem', color: 'var(--muted)', fontStyle: 'italic' }}>
        Sorted by trust and what's nearby - never by who paid.
      </p>
